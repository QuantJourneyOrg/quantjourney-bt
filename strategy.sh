...
usage() {
    ...
    echo "  No reports/plots/PDF:  $0 example_weights_01_sma_daily --no-reports"
    ...
    echo "      --no-reports       Skip text report, plots, and PDF; keep archive metadata"
    echo "  -o, --output DIR       Save reports/archive under DIR/<strategy_name>/"
    ...
    echo "  QJ_NO_REPORTS    1 skips report/plot/PDF generation"
    echo "  QJ_OUTPUT_DIR    Output root for reports/archive"
    echo "  QJ_SAVE_PICKLE_ARCHIVE  1 saves portfolio/instrument/blotter .pkl debug artifacts"
    exit "$status"
}
