...
def optimization_summary_dict(result: OptimizationResult) -> dict[str, Any]:
    ...
    if result.convergence_curve:
        ...
        improvements = sum(1 for i in range(1, len(curve)) if curve[i] > curve[i - 1])
        last_improve = 0
        ...
        early_improvements = sum(
            1 for i in range(1, min(mid_point, len(curve))) if curve[i] > curve[i - 1]
        )
        late_improvements = sum(1 for i in range(mid_point, len(curve)) if curve[i] > curve[i - 1])
    ...
    out["param_importance"] = (
        dict(sorted(result.param_importance.items(), key=lambda x: x[1], reverse=True))
    ...
    if len(completed) >= 5:
        ...
        out["stability"] = {
            ...
            "is_stable": top5_std < abs(result.best_objective) * 0.05
            if result.best_objective
            else True,
        }
    else:
        out["stability"] = {}
...
def _build_diagnostics(
    ...
    if param_imp:
        ...
        if low_imp:
            _add(
                "SUGGESTION",
                "PARAM_LOW_IMPORTANCE",
                f"Parameters with negligible importance (<5%): {', '.join(low_imp)}.",
                ...
                if best_val is not None and lo is not None and hi is not None:
                    ...
                    if rng > 0:
                        ...
                        if at_upper:
                            ...
                            _add(
                                "WARNING",
                                "PARAM_BOUNDARY_LOWER",
                                f"Best '{pname}' = {_fmt(best_val)} is at the LOWER "
                                f"boundary of [{lo}, {hi}].",
                                "The optimum may lie below your search range. "
                                "Extend the lower bound and re-run.",
                            )
    ...
    if stab.get("is_stable") and len(unstable_params) >= 2:
        _add(
            "WARNING",
            "OVERFIT_STABLE_OBJ_UNSTABLE_PARAMS",
            "Objectives are stable but parameters are spread — flat objective surface detected.",
    ...
    if meta.get("sampler") == "tpe" and result.n_evaluated < 30:
        _add(
            "INFO",
            "QUALITY_FEW_TRIALS_TPE",
            f"TPE sampler works best with 50+ trials. Only {result.n_evaluated} completed.",
...
def optimization_summary(
    ...
    *,
    ...
    lines.append(
        f"  Total Trials:      {ov['total_trials']}"
        f"  (✓ {ov['completed']}  ✂ {ov['pruned']}  ✗ {ov['failed']})"
    )
    lines.append(
        f"  Wall Time:         {ov['elapsed_human']}  ({ov['trials_per_second']:.1f} trials/sec)"
    )
    ...
    if cfg["sampler"] == "tpe":
        lines.append("                     (multivariate=True, constant_liar=True)")
    ...
    lines.append(
        f"  N Trials:          {cfg['n_trials_requested']}  (completed: {ov['completed']})"
    )
    ...
    if conv:
        ...
        lines.append(
            f"  1st Half Gain:            {_fmt(conv['first_half_gain'])}"
        ...
        if conv.get("fully_converged"):
            lines.append("  ✓ Study appears fully converged")
        else:
            lines.append("  → Study may benefit from additional trials")
    ...
    if ranges:
        ...
        for pname, info in ranges.items():
            if info["type"] == "numeric":
                ...
                lines.append(
                    f"    Best Region:   [{_fmt(info['best_region_min'])} → "
                ...
                for cat, cnt in sorted(dist.items(), key=lambda x: x[1], reverse=True):
                    ...
                    lines.append(f"    {str(cat):>20s}  {bar}  {cnt}x  (top-10%: {top_cnt}x)")
    ...
    if top:
        ...
        # Header
        lines.append(
            f"  {'Rank':>4}  {'Trial':>5}  {'Objective':>12}  {'Duration':>10}  Parameters"
        )
        lines.append(f"  {'─' * 4}  {'─' * 5}  {'─' * 12}  {'─' * 10}  {'─' * 30}")
        for t in top:
            params_str = ", ".join(
                f"{k}={_fmt(v) if isinstance(v, float) else v}" for k, v in t["params"].items()
            )
            lines.append(
                f"  {t['rank']:>4}  #{t['number']:>4}  "
    ...
    if stab:
        ...
        if stab.get("is_stable"):
            lines.append("  ✓ Optimal region is STABLE — results are reliable")
        else:
            lines.append("  ⚠ Optimal region shows HIGH VARIANCE — results may be noisy")
    ...
    if diags:
        ...
        sev_summary = "  ".join(
            ...
            for s, c in sorted(sev_counts.items(), key=lambda x: _SEVERITY_ORDER.get(x[0], 99))
        )
    ...
    lines.append("")
    lines.append("─" * width)
    lines.append("  Generated by QuantJourney Optimization Engine")
    lines.append(
...
def _add_ascii_histogram(
    lines: list[str], result: OptimizationResult, width: int = 50, height: int = 12
):
...
def _add_ascii_convergence(
    lines: list[str], result: OptimizationResult, width: int = 55, height: int = 10
):
