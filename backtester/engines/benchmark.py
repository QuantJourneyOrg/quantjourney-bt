    ...
    *,
    ...
    if "close" in columns:
        if not assume_close_adjusted:
            logger.warning(
                f"Benchmark {symbol} from {source} has no adjusted close; "
                "using raw close price-return series."
            )
...
def _align_benchmark_returns(
    benchmark_returns: pd.Series, returns_index: pd.DatetimeIndex
) -> pd.Series:
...
async def _fetch_benchmark_via_sdk(
    ...
    try:
        ...
        if date_col is None:
            logger.warning(
                f"Warehouse benchmark response missing date/close columns: {list(df.columns)}"
            )
            return None
        ...
        if price is None:
            logger.warning(
                f"Warehouse benchmark response missing price columns: {list(df.columns)}"
            )
            return None
        ...
        if "Unauthorized" in message or "no refresh token" in message:
            logger.debug(
                f"SDK warehouse benchmark unavailable for {symbol}; trying fallback providers."
            )
        else:
